...
Trojan-Go 服务端兼容所有支持标准 Trojan 协议的客户端。以下是目前仍在活跃维护的客户端：
...
> **注意：** 以上客户端支持标准 Trojan 协议。Trojan-Go 扩展特性（WebSocket 传输、smux 多路复用、Shadowsocks AEAD 二次加密）需要直接运行 `trojan-go` 二进制文件。此前内置 Trojan-Go 核心的图形客户端（[Qv2ray](https://github.com/Qv2ray/Qv2ray) 和 [Igniter-Go](https://github.com/p4gefau1t/trojan-go-android)）均已停止维护。
