...
Trojan-Go-Next 支持在 TLS 传输层之上使用 WebSocket 承载 Trojan 协议。这使得利用 CDN（如 Cloudflare）进行流量中转成为可能——客户端连接到 CDN 节点，CDN 再将 WebSocket 流量回源到你的 Trojan-Go-Next 服务器。

```
+-----------+     TLS      +---------+     回源      +------------+
| Trojan-Go-Next | ----------> |   CDN   | ----------> | Trojan-Go-Next  |
...
2. 在 DNS 记录中添加一条 A 记录：
   ...
   - IPv4 地址：你的 Trojan-Go-Next 服务器 IP
...
1. Trojan-Go-Next 服务端和客户端配置中同时启用 `websocket` 选项
...
```bash
# 1. 启动 Trojan-Go-Next 服务端
trojan-go-next -config server-config.json

# 2. 启动 Trojan-Go-Next 客户端
trojan-go-next -config client-config.json
