...
Trojan-Go 支持可插拔的传输层插件，可以替换默认的 TLS 传输层。开启插件后，Trojan-Go 客户端会将**未经 TLS 加密的明文流量**交给本地插件处理，由插件负责加密、混淆和传输；服务端插件接收流量后解密，再将明文流量交给本地 Trojan-Go 服务端。

```
+-----------+   明文   +-------------+   加密传输   +-------------+   明文   +-----------+
| Trojan-Go | ------> | 客户端插件   | ----------> | 服务端插件   | ------> | Trojan-Go |
| 客户端     |         | (加密/混淆)  |             | (解密/还原)  |         | 服务端     |
+-----------+         +-------------+             +-------------+         +-----------+
```
...
::: danger
开启可插拔传输层后，Trojan-Go 默认的 TLS 加密将被**完全替换**。如果插件本身不提供加密，流量将以明文传输。请务必选择具备加密能力的插件，否则你的代理密码和流量内容将完全暴露。
:::
...
首先下载 [v2ray-plugin](https://github.com/shadowsocks/v2ray-plugin) 并放置在 Trojan-Go 同目录下。
...
Trojan-Go 的 `shadowsocks` 插件类型完全兼容 SIP003 标准。启用后，Trojan-Go 会按照 SIP003 规范设置环境变量（如 `SS_LOCAL_HOST`、`SS_REMOTE_HOST` 等），并自动调整 `remote_addr`/`remote_port`/`local_addr`/`local_port`，使插件直接与远端通讯，Trojan-Go 仅与本地插件交互。

## 已验证可用的插件

以下插件经过社区验证，可与 Trojan-Go 配合使用：
...
::: warning
目前现有的插件均无法对接 Trojan-Go 的主动探测防御特性。如果你对安全性有较高要求，建议自行设计协议并开发相应插件。开发指南请参考"实现细节和开发指南"章节。
:::

## 验证方法

```bash
# 1. 启动 Trojan-Go，观察插件是否正常启动
