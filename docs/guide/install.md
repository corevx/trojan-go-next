...
本文将引导你从零开始部署 Trojan-Go。无论你是第一次接触代理工具，还是从其他工具迁移过来，都可以按以下步骤快速上手。
...
| TLS 证书 | 可通过 Let's Encrypt 免费申请，Trojan-Go 也支持自签证书 |
| 域名解析 | 将域名 A 记录指向你的服务器 IP |

::: tip
Trojan-Go 兼容原版 Trojan 协议。如果你之前使用过原版 Trojan，可以直接迁移配置文件，无需修改。
:::
...
从 [Release 页面](https://github.com/corevx/trojan-go-next/releases) 下载对应平台的压缩包，解压后即可使用。
...
Trojan-Go 依赖 TLS 加密，你需要一个有效的 TLS 证书。推荐使用 Let's Encrypt 免费证书：
...
::: tip
`remote_addr:remote_port` 指向一个本地 HTTP 服务。当非 Trojan 流量（如浏览器访问、GFW 主动探测）到达时，Trojan-Go 会将请求转发到这个 HTTP 服务，使其看起来像一个正常的 HTTPS 网站。
:::

启动服务端：

```shell
./trojan-go -config /etc/trojan-go/server.json
```
...
如果你使用 Linux 服务器，推荐将 Trojan-Go 配置为 systemd 服务，实现开机自启和自动重启。
...
这会自动：
- 将二进制文件安装到 `/usr/bin/trojan-go`
- 将示例配置复制到 `/etc/trojan-go/`
- 安装 systemd 服务文件
...
2. 创建 systemd 服务文件 `/usr/lib/systemd/system/trojan-go.service`：
...
::: tip
Trojan-Go 的代理端口同时支持 SOCKS5 和 HTTP 协议，无需额外配置。
:::
