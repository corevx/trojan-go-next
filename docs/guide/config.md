---
title: "正确配置Trojan-Go-Next"
---

本页介绍 Trojan-Go-Next 配置文件的整体结构与核心概念。如果你需要查看所有字段的完整说明，请参考 [完整配置文件](./full-config.md)。

## 配置格式：JSON 与 YAML

Trojan-Go-Next 同时支持 JSON 和 YAML 两种配置格式，二者结构等价。YAML 格式中字段名使用横杠（`-`）替代下划线（`_`），例如 `remote_addr` 写作 `remote-addr`。
...
Trojan-Go-Next 通过 `run_type` 字段决定工作模式。不同模式的含义和数据流向差异较大：
...
- **服务端（server）**：指向 HTTP 回落地址。当 TLS 握手成功但流量被识别为非 Trojan 协议时（例如浏览器访问、GFW 主动探测），Trojan-Go-Next 会将连接转发到该地址，使服务器表现与正常 HTTPS 网站一致。
...
```bash
trojan-go-next -config ./base.json -config ./override.json
```
...
```bash
# 客户端
trojan-go-next -client -remote your-server.com:443 -local 127.0.0.1:1080 -password s3cret

# 服务端
trojan-go-next -server -local 0.0.0.0:443 -remote 127.0.0.1:80 -password s3cert -cert server.crt -key server.key
```
...
通过 Trojan-Go-Next URI 启动，适合从分享链接直接连接：

```bash
trojan-go-next 'trojan-go-next://s3cret@your-server.com:443?sni=your-server.com#my-server'
```

URI 格式遵循 `trojan-go-next://password@host:port?parameters#name` 的结构。
