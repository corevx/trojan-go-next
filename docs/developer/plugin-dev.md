...
Trojan-Go-Next 鼓励开发传输层插件，以丰富协议类型，增加对抗主动探测的战略纵深。传输层插件替代 TLS 层进行传输加密和混淆，与 Trojan-Go-Next 本体通过 TCP Socket 通信，完全解耦。

## 设计原则

Trojan-Go-Next 的插件设计原则与 Shadowsocks SIP003 有所不同，遵循以下三条：

### 1. 加密、验证与抗重放

插件必须对传输内容进行加密、混淆和完整性校验，并能抵抗重放攻击。

这是因为 Trojan 协议本身不加密。将 TLS 替换为传输层插件后，Trojan-Go-Next 将 **完全信任插件的安全性**。TLS 提供的所有安全属性（机密性、完整性、身份认证、前向安全）需要由插件保证。
...
服务端的插件在检测到内容被篡改或遭到重放时，**必须将此连接交由 Trojan-Go-Next 处理**，而不是直接断开。具体步骤：

1. 将已读入和未读入的内容一并发送给 Trojan-Go-Next
2. Trojan-Go-Next 将连接重定向到一个真实的 X 服务器
3. 攻击者开始与真实的 X 服务器交互

这充分利用了 Trojan-Go-Next 的抗主动探测特性。即使 GFW 对服务器进行主动探测，服务器也能表现得与真实 X 服务一致。
...
3. 插件将连接（含探测载荷）交给 Trojan-Go-Next
4. Trojan-Go-Next 将连接重定向到真实 MySQL 服务器
...
Trojan-Go-Next 内部的隧道接口定义于 `tunnel/tunnel.go`，插件无需实现这些接口，而是通过 TCP Socket 与 Trojan-Go-Next 通信：
...
插件通过配置项 `plugin` 和 `plugin_opts` 激活，Trojan-Go-Next 会启动插件进程并将其标准输入/输出与 transport 层的 TCP 连接对接。

## SIP003 兼容性

Trojan-Go-Next 插件建议参照 [SIP003](https://shadowsocks.org/en/spec/Plugin.html) 标准开发。符合 SIP003 的插件可同时用于 Trojan-Go-Next 和 Shadowsocks。

### 环境变量

插件启动时，Trojan-Go-Next 通过环境变量传递连接信息：

| 环境变量 | 说明 |
|----------|------|
| `SS_LOCAL_HOST` | 本地监听地址（插件需要监听的地址） |
| `SS_LOCAL_PORT` | 本地监听端口 |
| `SS_REMOTE_HOST` | 远端地址（Trojan-Go-Next 服务端地址） |
| `SS_REMOTE_PORT` | 远端端口 |
...
客户端侧：
  ...
  Trojan-Go-Next → TCP → Plugin Client → 自定义协议 → 网络

  SS_LOCAL_HOST:PORT = Trojan-Go-Next 连接的目标
  SS_REMOTE_HOST:PORT = Plugin 发出的目标


服务端侧：

  网络 → 自定义协议 → Plugin Server → TCP → Trojan-Go-Next

  SS_LOCAL_HOST:PORT = Plugin 监听地址
  SS_REMOTE_HOST:PORT = Trojan-Go-Next 监听地址
```
...
插件进程启动后，通过 `SS_LOCAL_HOST:SS_LOCAL_PORT` 与 Trojan-Go-Next 通信，通过 `SS_REMOTE_HOST:SS_REMOTE_PORT` 与远端通信。
...
```
Trojan-Go-Next ←→ SS_LOCAL ←→ Plugin ←→ SS_REMOTE ←→ 网络
                 TCP          自定义协议           TCP
```
...
1. 启动 Trojan-Go-Next 服务端（不使用插件）
2. 启动插件服务端模式，指向 Trojan-Go-Next 监听地址
3. 启动插件客户端模式
4. 配置 Trojan-Go-Next 客户端连接到插件客户端地址

```bash
# 终端 1: Trojan-Go-Next 服务端
trojan-go-next -config server.json  # 监听 127.0.0.1:8443
...
# 终端 4: Trojan-Go-Next 客户端连接到 127.0.0.1:1080
```
